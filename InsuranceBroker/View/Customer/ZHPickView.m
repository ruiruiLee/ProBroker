...
-(void)remove{
    if(self.delegate && [self.delegate respondsToSelector:@selector(toobarCandelBtnHaveClick:)]){
        [self.delegate toobarCandelBtnHaveClick:self];
    }
...
-(void)doneClick
{
    ...
    if ([self.delegate respondsToSelector:@selector(toobarDonBtnHaveClick:resultDate:)]) {
        [self.delegate toobarDonBtnHaveClick:self resultDate:date];
    }
    [self removeFromSuperview];
}
