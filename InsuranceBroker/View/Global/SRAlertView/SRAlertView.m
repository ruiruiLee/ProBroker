...
#define kMessageLabelColor              UICOLOR_FROM_HEX_ALPHA(0x000000, 1.0)

#define kBtnNormalTitleColor            UICOLOR_FROM_HEX_ALPHA(0x000000, 1.0)
#define kBtnHighlightedTitleColor       UICOLOR_FROM_HEX_ALPHA(0x4A4A4A, 1.0)
#define kBtnHighlightedBackgroundColor  UICOLOR_FROM_HEX_ALPHA(0xFF6619, 1.0)

#define kLineBackgroundColor            UICOLOR_FROM_HEX_ALPHA(0xE3E3E3, 1.0)

#pragma mark - Use Frame

#define kAlertViewW             275.0f
#define kAlertViewTitleH        18.0f
...
#define kBtnTitleFont   [UIFont systemFontOfSize:SCREEN_ADJUST(15)];
...
- (UIView *)blurView {
    ...
    if (!_blurView) {
        _blurView = [[UIView alloc] initWithFrame:SCREEN_BOUNDS];
...
- (void)setupAlertView {
    ...
    [_alertView addSubview:({
        ...
        if(!_titleLabel){
            _messageLabel.frame = CGRectMake(messageLabelSpacing, CGRectGetMaxY(_titleLabel.frame) + verticalMargin + 12,
                                             kAlertViewW - messageLabelSpacing * 2, messageLabH);
        }
        _messageLabel;
    })];
    ...
    if(!_titleLabel){
        _alertView.frame  = CGRectMake(0, 0, kAlertViewW, CGRectGetMaxY(_messageLabel.frame) + kAlertViewBtnH + verticalMargin + 12 );
    }
...
//            [_rightAction setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
            [_rightAction setTitleColor:kBtnHighlightedTitleColor forState:UIControlStateHighlighted];
            [_rightAction setBackgroundImage:[self imageWithColor:kBtnHighlightedBackgroundColor] forState:UIControlStateNormal];
