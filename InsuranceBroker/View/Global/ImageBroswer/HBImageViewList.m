...
-(void)addImagesURL:(NSArray*)urls withSmallImage:(NSArray*)images
{
    ...
    for(int i=0; i < count; i++)
    {
        HBImageScroller * scroll=[[HBImageScroller alloc]initWithFrame:CGRectMake(self.bounds.size.width*i,0 , self.bounds.size.width,self.bounds.size.height)];
        [self addSubview:scroll];
        UIImage *image = nil;
        if(i<[images count])
            image=[images objectAtIndex:i];
        [scroll setImageWithURL:[urls objectAtIndex:i] andSmallImage:image];
        [_imageViews addObject:scroll];
        [scroll addTarget:self tapOnceAction:@selector(tapImageAction:)];
...
-(void)setImage:(UIImage *)image
{
    ...
    self.contentOffset=CGPointMake( self.frame.size.width*offset, 0);
    _prePage=offset;
