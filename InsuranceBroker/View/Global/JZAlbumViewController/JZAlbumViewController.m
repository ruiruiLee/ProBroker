...
//    [[SDImageCache sharedImageCache] clearMemory];
    self.scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, SCREEN_HEIGHT)];
    self.scrollView.pagingEnabled = YES;
    self.scrollView.userInteractionEnabled = YES;
    self.scrollView.showsHorizontalScrollIndicator = NO;
    
    self.scrollView.contentSize = CGSizeMake(self.imgArr.count*ScreenWidth, SCREEN_HEIGHT);
...
-(void)loadPhote:(NSInteger)index{
    ...
    if (![currentPhotoView isKindOfClass:[PhotoView class]]) {
        ...
        if([obj isKindOfClass:[NSString class]]){
            PhotoView *photoV = [[PhotoView alloc] initWithFrame:frame withPhotoUrl:obj];
            photoV.delegate = self;
            [self.scrollView insertSubview:photoV atIndex:0];
            [_subViewList replaceObjectAtIndex:index withObject:photoV];
        }else{
            PhotoView *photoV = [[PhotoView alloc] initWithFrame:frame withPhotoImage:obj];
            photoV.delegate = self;
            [self.scrollView insertSubview:photoV atIndex:0];
            [_subViewList replaceObjectAtIndex:index withObject:photoV];
        }
...
-(void)pinGes:(UIPinchGestureRecognizer *)sender{
    ...
    self.scrollView.contentSize = CGSizeMake(self.imgArr.count*ScreenWidth, SCREEN_HEIGHT*lastScale);
